...
export function OtpGate({ identifier }: { identifier: string }) {
  const router  = useRouter();
  const mode: Mode = isEmail(identifier) ? 'email' : 'whatsapp';
  ...
  const sendEndpoint = mode === 'email' ? '/api/auth/otp/email' : '/api/auth/otp/send';
  const sendBody     = mode === 'email'
    ? { email: identifier }
    : { phone: identifier };
      ...
      <div className="w-full max-w-sm flex flex-col gap-6">
        <div className="flex flex-col gap-2">
          ...
          <p className="text-sm leading-relaxed" style={{ color: 'var(--muted)', fontFamily: 'var(--font-dm-sans)' }}>
            We&apos;ll send a 6-digit code to your {label}.
          </p>
        </div>
        ...
        {step === 'idle' && (
          <button
            ...
            style={{
              background: mode === 'email' ? 'var(--coral)' : '#25D366',
              color: '#fff',
          ...
          >
            ...
            {sending ? 'Sending…' : `Send code via ${label}`}
          </button>
        )}
        ...
        {error && (
          <p className="text-sm text-center" style={{ color: 'var(--coral)', fontFamily: 'var(--font-dm-sans)' }}>{error}</p>
        )}
      </div>
    </main>
  );
