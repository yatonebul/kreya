...
export default function ReelEditor({ params, searchParams }: PageProps) {
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        if (!data.timeline) {
          // Single-image GPU post — show full editor using rerender endpoint
          setNoTimeline(true);
  ...
  // ── Build updated timeline (multi-clip path) ───────────────────────────────
  ...
  const buildUpdatedTimeline = useCallback((): KreyaTimeline | null => {
    ...
    const captions = captionOn && captionText.trim()
      ? [{ text: captionText.trim(), startTime: 0, duration: timeline.totalDuration, position: 'bottom' as const, platform: 'ig-reels' as const }]
      : undefined;
    return { ...timeline, colorGrade: selectedGrade, bgStyle, tracks: { ...timeline.tracks, video: updatedVideo, captions: captions ?? undefined } };
  }, [timeline, selectedGrade, bgStyle, motionStyle, captionOn, captionText]);
  ...
  async function applyChanges() {
    ...
    try {
      if (noTimeline || !timeline) {
        // Single-image path — use rerender endpoint
        const res = await fetch(`/api/posts/${postId}/rerender`, {
          method:  'POST',
          headers: { 'Content-Type': 'application/json' },
          body:    JSON.stringify({ token, phone, animationStyle: motionStyle, musicPreference: musicPref, colorGrade: selectedGrade, bgStyle, captionText: captionOn ? captionText.trim() : undefined }),
        });
        const data = await res.json();
      ...
      // Multi-clip timeline path — include musicPreference so update-timeline can swap music
  ...
  return (
    <div className="min-h-screen pb-28" style={{ background: '#0B0918', color: '#fff', fontFamily: 'DM Sans, sans-serif' }}>
      ...
      <div className="sticky top-0 z-10 flex items-center justify-between px-4 py-3" style={{ background: '#100E22', borderBottom: '1px solid rgba(255,255,255,.08)' }}>
        <span className="font-semibold" style={{ fontSize: '1rem', fontFamily: 'Syne, sans-serif' }}>
          {noTimeline ? 'Edit Reel' : 'Edit Reel'}
        </span>
      ...
      <div className="px-4 mt-6 space-y-6">
        ...
        {/* ── Clip order (multi-clip timeline only) ── */}
